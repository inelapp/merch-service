			...
			if (result instanceof JsonWebTokenError) {
				switch (result.message) {
					case 'jwt must be provided':
						return err(new UserRefreshTokenMissingError());
					case 'invalid token':
						return err(new UserRefreshTokenInvalidTokenError());
					case 'jwt expired':
						return err(new UserRefreshTokenExpiredTokenError());
					case 'jwt malformed':
						return err(new UserRefreshTokenMalformedTokenError());
					case 'jwt signature is required':
						return err(new UserRefreshTokenRequiredSignatureError());
					case 'invalid signature':
						return err(new UserRefreshTokenInvalidSignatureError());
				}
			}
